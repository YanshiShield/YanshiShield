...
class Job:
        ...
        if self.__status.state is State.STARTING:
            try:
                ...
                yield self.__coordinator.create(
                    ...
                    workspace=os.path.join(const.JS_WORKSPACE_PATH,
                                           const.JS_DIR_NAME_IN_HOST),
    ...
    def assert_model_path_existing(self, model_path):
        ...
        Raises:
            ModelNotExist: raise if the model not existing.
        """
        model_absolute_path = os.path.join(const.JS_WORKSPACE_PATH,
                                           self.__namespace,
                                           model_path.lstrip("/"))
    ...
    def assert_checkpoint_existing(self, checkpoint_id):
        ...
        ckpt_absolute_path = os.path.join(const.JS_WORKSPACE_PATH,
                                          self.__namespace,
    ...
    def __do_delete_ckpts(self):
        ouput_root = os.path.join(const.JS_WORKSPACE_PATH,
                                  self.__namespace,
        ...
        for dir_name in dirs:
            if re.search(job_dir_pattern, dir_name):
                shutil.rmtree(os.path.join(
                    const.JS_WORKSPACE_PATH, self.__namespace,
                    self.__job_config["output"].lstrip("/"),
                    dir_name))
