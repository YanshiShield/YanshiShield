...
class TestSSAProtector(unittest.TestCase):
    ...
    def test_should_success_encrypt_int(self):
        ...
        self.assertEqual(result,
                         1 * PLAINTEXT_MULTIPLE + self.prg.next_value())

    def test_should_success_encrypt_float(self):
        result = self.protector.encrypt(1.123)

        self.assertEqual(result,
                         1.123 * PLAINTEXT_MULTIPLE + self.prg.next_value())
    ...
    def test_should_success_encrypt_np_array(self):
        ...
        self.assertTrue(
            self.__equal(
                new_array, np.full_like(
                    new_array, (1 * PLAINTEXT_MULTIPLE
                                + self.prg.next_value(np_array.shape)))))
    ...
    def test_should_success_encrypt_list(self):
        ...
        self.assertTrue(self.__equal(
            new_array_list[0],
            np.full((2, 2, 3), (1 * PLAINTEXT_MULTIPLE
                                + self.prg.next_value((2, 2, 3))))))
        self.assertTrue(self.__equal(
            new_array_list[1],
            np.full_like(new_array_list[1],
                         (2 * PLAINTEXT_MULTIPLE
                          + self.prg.next_value((3, 1, 3))))))
    ...
    def test_should_success_encrypt_dict(self):
        ...
        self.assertEqual(new_dict['int'],
                         1 * PLAINTEXT_MULTIPLE + self.prg.next_value())
        self.assertEqual(new_dict['float'],
                         1.1 * PLAINTEXT_MULTIPLE + self.prg.next_value())
        self.assertTrue(self.__equal(
            new_dict['array'],
            np.full_like(new_dict['array'],
                         (1 * PLAINTEXT_MULTIPLE
                          + self.prg.next_value((1, 2, 3))))))
