...
class TestSSAProtector(unittest.TestCase):
    ...
    def test_should_success_encrypt_int(self):
        ...
        self.assertEqual(result, 1 + self.prg.next_number())
    ...
    def test_should_success_encrypt_np_array(self):
        ...
        self.assertTrue(
            self.__equal(new_array,
                         np.full_like(new_array, 1 + self.prg.next_number())))
    ...
    def test_should_success_encrypt_list(self):
        ...
        self.assertTrue(self.__equal(
            new_array_list[0], np.full((2, 2, 3), 1 + self.prg.next_number())))
        self.assertTrue(self.__equal(
            new_array_list[1],
            np.full_like(new_array_list[1], 2 + self.prg.next_number())))
    ...
    def test_should_success_encrypt_dict(self):
        ...
        self.assertEqual(new_dict['int'], 1 + self.prg.next_number())
        self.assertEqual(new_dict['float'], 1.1 + self.prg.next_number())
        self.assertTrue(self.__equal(
            new_dict['array'],
            np.full_like(new_dict['array'], 1 + self.prg.next_number())))
...
class TestSSAProtectorWithUseSameMask(unittest.TestCase):
    """Test class of SSA Protector.
    """
    def setUp(self):
        self.protector = SSAProtector("test", True)
